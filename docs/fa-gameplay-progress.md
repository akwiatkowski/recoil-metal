...
| [`FA-MATCH`](#fa-match---armies-setup-and-victory-rules) | Armies, setup, victory rules | `WP-10`-`11` | 85% | 50% | 85% | Scenario Options wired end to end: `Victory` drives `Match::victoryMode`, `FogOfWar='none'` leaves intel unconfigured, and `UnitCap` now sets `Army::unitCap` — the retail creation gate (`0x0074fda0`) is implemented: `General.CapCost` parses (default 1), factory production holds and retries at the cap, mobile scaffolds/upgrades refuse, `UnitCap_Current`/`UnitCap_MaxCap` feed `CArmyStats`. Draw offers in (`C-382`). Remaining: `InitialMass`/`InitialEnergy` starting storage. |
...
`FogOfWar='none'` leaves intel unconfigured. **Unit caps are now in**: `General.CapCost`
parses (engine default 1; the corpus only ever overrides down — drones/walls at 0,
T1 sonar at 0.1), `Options.UnitCap` sets `Army::unitCap` (engine default 500,
`0x008e8035`), and the retail creation gate (`0x0074fda0`,
`costTotal + reserved + CapCost > unitCap`) is implemented — factory production
holds and retries like `CFactoryBuildTask`, mobile scaffolds and upgrades are
refused outright, and `UnitCap_Current`/`UnitCap_MaxCap` feed `CArmyStats` for
`aibrain.lua`'s score row and `AIBehaviors.lua`'s experimental gate.
`SimUtils.UpdateUnitCap` is a literal no-op in retail (`floor(alive × initial /
alive)` = `initial`), so `DoNotShareUnitCap` needs no consumer. Draw offers are
in (`C-382`), the 15-second winner stability window tracks the survivor SET
(`pendingSurvivorMask`), and the 20-second post-defeat cleanup is already in.
`InitialMass`/`InitialEnergy` remain the only open WP-11 gap.

```text
/goal Advance FA-MATCH by closing the last WP-11 gap: `InitialMass`/`InitialEnergy`
(`Options` → starting stored resources) still parse without a consumer — recover
the retail starting-storage mechanic before wiring the parsed option. Preserve
the 3-second poll cadence and C-210's predicates, run make test and make verify,
and refresh FA-MATCH.
```
