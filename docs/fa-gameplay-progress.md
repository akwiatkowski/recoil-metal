...
```text
Implemented       [##############------] about 70%
Retail-validated  [########------------] about 40%
Retail-analyzed   [##################--] about 88%
```

The row estimates average 69.53%, 39.84% and 87.89%, respectively. The 2026-09-18
...
`FA-PROGRESS` (40→85 analyzed) on the full enhancement lifecycle (`C-251`–`C-265`, `C-376`–`C-381`),
`FA-TERRAIN` (45→85 analyzed) on `FlattenMapRect`/wreck obstruction/terrain types/scorch
(`C-286`–`C-292`), `FA-PRESENT` (45→85 analyzed) on manipulators/effects/audio/LOD
boundary (`C-293`–`C-304`, `C-370`–`C-375`), `FA-LUA` (40→85 analyzed) on the full LuaPlus dialect,
two-ring FIFO scheduler, script lifecycle, and mod-hook contract (`C-305`–`C-319`),
`FA-NAVY` (75→85 analyzed) on the water mover, weapon gating, spawn layers and naval
pathing (`C-320`–`C-328`), `FA-CMD` (75→85 analyzed) on guard task flags, the ferry
contract, toggle dispatch and factory intake (`C-347`–`C-351`), `FA-AI` (30→85
analyzed) on the complete native AI object model, threat grid and Lua boundary
(`C-352`–`C-362`), and `FA-UI` (15→85 analyzed) on the command-sink wire alphabet,
RULEUCC→UNITCOMMAND map, selection flags and sim→UI mirror (`C-329`–`C-346`,
`C-363`–`C-369`).
...
The independent evidence-state count is **40 of 45 WPs at `Analyzed`**. That 89% inventory count
and the 88% equal-subsystem estimate answer different questions and are shown together to keep the
headline honest.
...
| [`FA-MATCH`](#fa-match---armies-setup-and-victory-rules) | Armies, setup, victory rules | `WP-10`-`11` | 95% | 50% | 85% | WP-11 closed (`C-383`/`C-384`): `UnitCap` gates creation (factory holds, scaffolds refuse), and the starting bank is the ACU's own `Economy.Storage*` granted on the fifth beat (`GiveInitialResources`, `WaitTicks(5)` `n−1` quirk) — `InitialMass`/`InitialEnergy` proved dead options. Commander storage counts toward capacity; ACU death drops the ceiling and the bank. `Victory`/`FogOfWar`/`OfferDraw` already wired. Remaining axis: retail-validation, not gaps. |
...
| [`FA-AIR`](#fa-air---aircraft-flight-combat-and-staging) | Aircraft flight, combat, staging | `WP-22` | 65% | 60% | 95% | Retail winged controller (C-221/222/223/244–247) with states 1–7, banking, cargo mass ratio, bomb-drop prediction, staging-pad refuel and carrier storage pools (`C-225`); a takeoff committed to a sub-tick hop no longer strands `Down` on the deck (`c538105`). Three-axis solver, `Hover`, `POD` init, bone-keyed air staging stay open. |
...
| [`FA-MISSILES`](#fa-missiles---silos-missiles-and-interception) | Silos, missiles, interception | `WP-29` | 70% | 45% | 95% | Manual missile-launch orders in with UI wiring and tests (`8d33735`); interceptor lead landed earlier; the silo build queue with assist links is in (`f41445e`). Strategic-nuke silos now launch — `siloLaunched()` asked `damage > 0` and a nuke's authored `Damage` is 0 (the warhead is `NukeInner/OuterRingDamage`), so the gate now asks `harmful()`. C-171 zig-zag weave implemented: `MaxZigZag`/`ZigZagFrequency` parsed, serial re-roll from the match stream, tapered triangle oscillation. The returned-missile friendly-fire channel is in: `redirectMissile` flips `CollideFriendly`/`DamageFriendly`/`DamageSelf` (the Lua's RedirectingState), the sweep admits the source side, and the impact path damages it including the launcher — SaveState v39 carries the flag and the weave schedule. Remaining: `0x005DEAD0` name, the 10-vs-11-tick redirect cooldown. |
...
| [`FA-TERRAIN`](#fa-terrain---mutable-terrain-and-craters) | Mutable terrain and craters | `WP-37` | 60% | 15% | 85% | **Analyzed 2026-09-18 (C-286–C-292):** `FlattenMapRect` writes uniform u16 elevation, dirty render heightfield, re-seats Land/Seabed entities — no pathing/ogrid. Wrecks use footprint-less `DefaultWreckage_prop.bp` and never block; `Physics.BlockPath` is a dead key. Terrain types gate pathing only via `Blocking` flag; `Slippery`/`Bumpiness`/`HealthEffectPerSecond` are dead. Scorch is visual-only, 28 corpus files, gated on `layer=='Land'`/`targetType=='Terrain'|'Prop'`, no underwater. **Implemented:** non-lethal impact scorch via `impactMarks` on `ProjectileImpact` Terrain/Prop events (C-291/C-292 tested), wreck marks as permanent `Feature` records, no height/type mutation on damage. Remaining: splat/decal scale split + texture variety + lifetimes (FA-PRESENT), beam `ScorchThread` trails and per-weapon decal tables (FA-LUA script hosting). |
...
**Largest gap:** WP-11 is now fully implemented on the analyzed contract. The
scenario Options contract is wired end to end — `resolveScmap` reads
`_scenario.lua` into `LoadedMap::scenarioOptions`, `makeMatchRunner` maps
`Options.Victory` through `victory.lua`'s own keys, `FogOfWar='none'` leaves
intel unconfigured, `UnitCap` gates creation (`C-383`: factory holds and
retries, scaffolds/upgrades refuse, `UnitCap_*` feeds `CArmyStats`), and the
starting bank is the ACU's own `Economy.Storage*` granted on the fifth beat
(`C-384`: `GiveInitialResources`, `WaitTicks(5)` `n−1` quirk; commander storage
counts toward capacity, so a dead ACU drops the ceiling and the bank).
`InitialMass`/`InitialEnergy` proved dead options — parsed at `0x8e7fef`, no
consumer in binary or corpus. Draw offers are in (`C-382`), the 15-second
winner stability window tracks the survivor SET (`pendingSurvivorMask`), and
the 20-second post-defeat cleanup is already in.

```text
/goal Advance FA-MATCH on the retail-validation axis: the WP-11 contract is
fully implemented — what remains is proving it against the executable. The
open questions are behavioural, not structural: does the winner-stability
window's survivor-set tracking match retail's `victory.lua` poll under
mid-window defeats, and does the cap gate's `OnUnitCapLimitReached` callback
surface anywhere the sim should mirror. Run make test and make verify, and
refresh FA-MATCH.
```
...
`BombDropThreshold` (`C-224`). Carrier storage is in as well (`C-225`):
`StorageSlots` is parsed, a `CARRIER`/`NAVALCARRIER` stores its air build
products at its origin instead of rolling them off, `(stored + reserved) <
StorageSlots` gates capacity, stored aircraft are hidden from intel, and
`UnloadTransport` on a carrier launches the hold airborne. `Hover`,
`POD`-only random initialization of the otherwise-zero `CUnitMotion+0x9c` elevation adjustment,
and bone-keyed air staging (`C-225`'s pad half) remain.
...
at each boundary. Remaining: the third `HasSiloAmmo` caller's name
(`0x005DEAD0`) and whether the redirect cooldown is 10 or 11 ticks.
...
**Largest gap:** non-lethal impact scorch is now in — `impactMarks` records a
scorch on every `ProjectileImpact` event typed `Terrain`/`Prop` (C-291/C-292
tested: water excluded, hash-invisible, unit hits mark the victim not the
ground). What remains is presentation fidelity, not sim state: the splat/decal
scale split (>1.2 → decal ×3, else splat ×4), the 10/2 texture lists, LOD
200–350 and 300–600 lifetimes (FA-PRESENT), and the per-weapon scripted decal
table plus beam `ScorchThread` trails (FA-LUA script hosting). Retail wrecks
never obstruct — features stay non-obstacles, matching `DefaultWreckage_prop.bp`.

```text
/goal Advance FA-TERRAIN on the presentation axis: the sim-side contract is
met (impact scorch records exist, height/pathing untouched). Next is the
splat/decal scale split with texture variety and lifetimes — FA-PRESENT work
projecting the existing `impactMarks`/`Feature` records — then beam
`ScorchThread` trails once the collision-beam scripts are hosted (FA-LUA).
```
