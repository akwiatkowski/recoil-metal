...
| [`FA-WEAPONS`](#fa-weapons---targeting-weapons-and-projectiles) | Targeting, weapons, projectiles | `WP-27`-`28` | 75% | 55% | 90% | Separate forced Attack targets from automatic acquisition (`recoil-metal-7599`). |
...
| [`FA-DAMAGE`](#fa-damage---damage-death-and-shields) | Damage, death, shields | `WP-30`-`32` | 60% | 40% | 75% | Route direct projectiles through collidable shield entities. |
| [`FA-INTEL`](#fa-intel---vision-radar-sonar-and-counter-intel) | Vision, radar, sonar, counter-intel | `WP-33` | 65% | 45% | 90% | Add remembered recon contacts and seen-now/seen-ever/maybe-dead state. |
...
**Largest gap:** retail issues one shared command to a canonical unit set; Recoil Metal still
has to prove its producer/replay cutover through the real app dispatcher. Slices 1-3 are complete;
slices 4-5 have implementation but incomplete acceptance evidence.

```text
/goal Advance FA-CMD by closing WP-12 slice 4's acceptance gap: add one app-level live-versus-replay
test through semantic submission, PreTick and PostSpawn dispatch, unit birth, and replay. Assert
identical accepted sets, source counters, creation serials, and hashes; mutate runtime queue state
and prove the recorded semantic input is unchanged. Run make test and make verify, then update
WP-12 and FA-CMD and make the slice-5 lifecycle matrix the next task.
```
