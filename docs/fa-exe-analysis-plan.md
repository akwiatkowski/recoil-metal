...
| `Moho::Entity` | `+0x148` | entity id (logged as `id=0x%08x`) | `int` | `C-045` agent trace of `Entity::Destroy` |
...
1. **`grep` silently drops matches in the retail corpus, and `LC_ALL=C` alone does not fix it.**
   ...
   **Use `grep -a` (and `LC_ALL=C`), or read the files in Python with `encoding='latin-1'`.**
