...
| `Moho::Unit` | `+0x544` | weapon container | — | `C-081` |
| `Moho::Unit` | `+0x4A0` | 64-bit unit-state mask; bit 45 = `SiloBuildingAmmo` | `u64` | `C-081` |
| `Moho::Unit` | `+0x2AC` | silo build progress 0..1 | `float` | `C-081` |
...
| `WP-27` | Weapon target acquisition, priorities, arcs, retargeting | Ready but not confirmed | High | **Analyzed** | `PE-15` | P1 | `C-090`–`C-095`: one cached per-**unit** candidate survey shared by all its weapons; selection ordered by **(range/arc class, priority index, score, incumbency)** with an exact tie falling through to unsorted grid order; the score is **turret slew, not distance, for turreted weapons**; cadence is `TargetCheckInterval`-driven and unstaggered; anti-projectile acquisition is a **separate algorithm**. No threat or value term exists anywhere. |
...
| `C-110` | `WP-30`, `WP-31` | **Our shield absorption is applied once to the whole blast and tested against the blast centre — the most consequential defect found in the campaign.** An agent built a probe against `librm_core.a` and measured it: one 100-point shield near a blast centre absorbed 40 and thereby saved **three separate units**, including one 150 elmos away and entirely outside the 80-elmo dome. Retail subtracts a per-shield amount inside the **per-entity** call, so each of those three takes its own damage less whatever shield actually covers *it*. Two further errors in the same block: coverage is computed from `centre − shieldCentre` rather than from the target, so a blast outside a dome damages a unit inside it at full strength; and a `break` after the first absorbing shield means **only one shield ever absorbs**, where retail walks the whole world shield list. | [RM] `src/core/sim/Combat.cpp:801-851`, probe-verified; [EXE] `C-060`, `C-062` | High | **Open — top priority** | This **silently cancels most of what the falloff fix restored**: `aa65554` doubled blast damage, and a single small shield anywhere near the centre still nullifies the whole area effect. Defects 1 and 2 are one rewrite of the shield block. |
