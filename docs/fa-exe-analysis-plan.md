...
| **Documented blueprint schema keys** (`C-165`) | **725** | with the engine's own descriptions |
...
| **The blueprint schema's own docs** (`C-165`) | **725 keys with the engine's one-line description of each**, in offset order | Found late; names any blueprint offset by position |
...
4. **The shipped-Lua corpus is not just `lua.scd`, and `build/re-fa/lua/` holds only that one
   ...
   overrides, and **`units.scd` 568 per-unit scripts**. A Tier-0 grep over `build/re-fa/lua/`
