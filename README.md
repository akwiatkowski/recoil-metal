...
- **Tested.** **1388 tests, all green.** Anything that does not
...
parity. The canonical [`Forged Alliance gameplay progress dashboard`](docs/fa-gameplay-progress.md)
shows every subsystem's estimated implementation, retail-validation, and retail-analysis
completion. Each stable subsystem ID also carries its exact next task and a copy-ready `/goal`
prompt. The detailed evidence behind those estimates remains in
[`docs/fa-exe-analysis-plan.md`](docs/fa-exe-analysis-plan.md).
...
make build
make test
#   100% tests passed, 0 tests failed out of 1388
```
...
| `--bench-hud` / `--bench-size <w> <h>` | make the offscreen benchmark draw the interface a screenshot of the same scene shows, and at a logical size other than 1920x1080; `--backing` applies too. World-only runs the identical headless composition with only the 2D HUD/minimap cleared, so subtraction does not change particles or icon fallbacks; it remains the default so published numbers stay comparable |
...
├── tests/              Catch2 unit tests, mirrors src/core — 1388 tests
