...
- **Tested.** **1089 tests across 98 files, all green.** Anything that does not
...
make build
make test
#   100% tests passed, 0 tests failed out of 1089
```
...
| `--ui faf` / `--ui-scale <0.5..3>` | wear the game's own chrome; request HUD scaling relative to automatic size. Values clamp; available fit wins, and enlargement uses only room beyond the automatic 2.5× cap |
...
├── tests/              Catch2 unit tests, mirrors src/core — 98 files, 1089 tests
