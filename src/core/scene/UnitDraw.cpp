...
[[nodiscard]] DrawUnit at(const sim::UnitView& view) noexcept {
    return DrawUnit{
        ...
        .turretYaw = sim::radiansFromBrad(view.turretYaw),
        .turretPitch = sim::radiansFromBrad(view.turretPitch),
        .turretYaw2 = sim::radiansFromBrad(view.turretYaw2),
        .turretPitch2 = sim::radiansFromBrad(view.turretPitch2),
...
void interpolate(const sim::Snapshot& from, const sim::Snapshot& to, float alpha,
    ...
    for (const sim::UnitView& now : to.units) {
        ...
        out.push_back(DrawUnit{
            ...
            .turretYaw = mixAngle(then.turretYaw, now.turretYaw, t),
            .turretPitch = mixAngle(then.turretPitch, now.turretPitch, t),
            .turretYaw2 = mixAngle(then.turretYaw2, now.turretYaw2, t),
            .turretPitch2 = mixAngle(then.turretPitch2, now.turretPitch2, t),
