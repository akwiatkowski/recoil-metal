...
std::expected<unitdef::UnitDef, lua::ParseError> load(std::string_view source,
    ...
    if (const lua::Value* defense = parsed->path("Defense")) {
        ...
        if (const lua::Value* shield = defense->find("Shield");
            ...
            // PersonalBubble shelters only its owner and TransportShield rides
            // transports (see ShieldSpec); both parse as ordinary shields with
            // their flags recorded. Retail's original UnitShield, however, is an
            // ordinary shield entity whose coverage primitive is an attached box
            // (`shield.lua:420-477`).
            const bool personal = flag("PersonalShield");
            def.shield.personalBubble = flag("PersonalBubble");
            def.shield.transportShield = flag("TransportShield");
            def.shield.maximum =
