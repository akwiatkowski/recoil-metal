...
# build order skipped the authorisation check and left no entry in the log — and P4.2's claim
# was true of movement and false of building, for months, with a green check saying "one path".
# A guard scoped to half the property is how that survives being noticed. The build path is
# routed now, and appending to a construction list is watched the same way an order is.
...
if [ -n "$hits" ]; then
    ...
    echo "so is pushing a Construction onto the building list by hand. The whole point is that"
    echo "there is only one — a human's click and a script's decision must not be merely"
    echo "similar."
    exit 1
fi
