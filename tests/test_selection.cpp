...
TEST_CASE("a plain click on a unit selects just it") {
    const std::vector<SelectionEntry> after = applyClick<SelectionEntry>({}, kA, /*addToSet=*/false);
...
TEST_CASE("a plain click on another unit replaces the selection") {
    ...
    const std::vector<SelectionEntry> after = applyClick<SelectionEntry>(current, kC, false);
...
TEST_CASE("a modified click adds to the selection") {
    ...
    const std::vector<SelectionEntry> after = applyClick<SelectionEntry>(current, kB, /*addToSet=*/true);
...
TEST_CASE("clicking a selected unit removes it, and leaves the rest") {
    ...
    const std::vector<SelectionEntry> after = applyClick<SelectionEntry>(current, kB, true);
    REQUIRE(after.size() == 2);
    CHECK(after[0] == kA);
    CHECK(after[1] == kC);
    ...
    const std::vector<SelectionEntry> plain = applyClick<SelectionEntry>(current, kB, false);
...
TEST_CASE("a plain click on empty ground clears the selection") {
    const std::vector<SelectionEntry> current{kA, kB};
    CHECK(applyClick<SelectionEntry>(current, std::nullopt, false).empty());
}
...
TEST_CASE("a modified click on empty ground keeps the selection") {
    ...
    const std::vector<SelectionEntry> current{kA, kB};
    const std::vector<SelectionEntry> after = applyClick<SelectionEntry>(current, std::nullopt, true);
...
TEST_CASE("selection order is stable") {
    ...
    current = applyClick<SelectionEntry>(current, kC, true);
    current = applyClick<SelectionEntry>(current, kA, true);
    current = applyClick<SelectionEntry>(current, kB, true);
