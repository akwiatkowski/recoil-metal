...
TEST_CASE("historic attachment saves derive offsets from their transforms", "[save-state]") {
    ...
    // V26's production-pause flags sit last inside the units payload, after
    // doNotTarget: one count plus one byte per slot. Removing the trailers above
    // leaves it at the tail, so it comes off with them.
    constexpr std::size_t kV26ProductionPausedBytes = sizeof(std::uint32_t) + kSlots;
    v7.resize(v7.size() - kV26ProductionPausedBytes - kV25DualPoseBytes - kV24TurretPoseBytes - kV23AbsentFeatureBytes - kV22BoneBytes - kV21BankBytes - kV20EmptyCapturesBytes - kV19EmptyEnhancementsBytes - kV18SubmarineBytes - kV16ControllerBytes - kV15AbsentEconomyBytes - kV14MotionBytes - kV10RedirectBytes - kV9SiloAmmoBytes
